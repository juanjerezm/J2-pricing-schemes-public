* NOTES:
* S2: free hourly pricing
* S3: uniform annual pricing
* S4: electricity-indexed pricing
* S5: temperature-indexed pricing (NOT FULLY USED, PROBABLE BUGS)

* C1: Small-DC,  1 MW, without transmission loss
* C2: Large-DC, 50 MW, with transmission losses
* ======================================================================
*  Options
* ======================================================================
* option limrow = 100
...
$ifi %case% == CA M_case    'Case multiplier'           /25/
$ifi %case% == CB M_case    'Case multiplier'           /25/
;
...
lambda_a(T)                         'Dual variable for eq_cold_balance'
lambda_b(T,I)                       'Dual variable for eq_production_cold'
lambda_c(T,I)                       'Dual variable for eq_production_HR'

u_a(T,I)                            'Auxiliary variable for linearisation'
u_b(T,I)                            'Auxiliary variable for linearisation'
u_c(T,I)                            'Auxiliary variable for linearisation'
u_d(T,I)                            'Auxiliary variable for linearisation'
u_e(T,I)                            'Auxiliary variable for linearisation'

$ifi %scenario% == S4   pi_alpha_0  'Intercept for waste-heat price (EUR/MWh)'
$ifi %scenario% == S4   pi_alpha_1  'Slope for waste-heat price (EUR/MWh)'
$ifi %scenario% == S5   pi_alpha_0  'Intercept for waste-heat price (EUR/MWh)'
$ifi %scenario% == S5   pi_alpha_1  'Slope for waste-heat price (EUR/MWh)'

$ifi %scenario% == S2   pi_hr(T)    'Waste-heat price (€/MWh) - indexed by timestep'
$ifi %scenario% == S3   pi_hr       'Waste-heat price (€/MWh) - constant'
$ifi %scenario% == S4   pi_hr(DD)   'Waste-heat price (€/MWh) - indexed by day'
$ifi %scenario% == S5   pi_hr(DD)   'Waste-heat price (€/MWh) - indexed by day'

* $ifi %scenario% == S4   pi_hr(WW)   'Waste-heat price (€/MWh) - indexed by week'
* $ifi %scenario% == S5   pi_hr(WW)   'Waste-heat price (€/MWh) - indexed by week'
* $ifi %scenario% == S4   pi_hr(MM)   'Waste-heat price (€/MWh) - indexed by month'
* $ifi %scenario% == S5   pi_hr(MM)   'Waste-heat price (€/MWh) - indexed by month'
;
...
mu_a(T,I)                           'Dual variable: eq_production_max_CO'
mu_b(T,I)                           'Dual variable: eq_production_max_HR'
mu_c(T,I)                           'Dual variable: eq_stationarity_c'
mu_d(T,I)                           'Dual variable: eq_stationarity_d'
mu_e(T,I)                           'Dual variable: eq_stationarity_e'
;

SOS1 Variables
nu_a(T,I,SOS)                       'Auxiliary variable for linearisation'
nu_b(T,I,SOS)                       'Auxiliary variable for linearisation'
nu_c(T,I,SOS)                       'Auxiliary variable for linearisation'
nu_d(T,I,SOS)                       'Auxiliary variable for linearisation'
nu_e(T,I,SOS)                       'Auxiliary variable for linearisation'
;
...
eq_stationarity_c(T,I)              'Stationarity condition: x_c'
eq_stationarity_d(T,I)              'Stationarity condition: x_hr'
eq_stationarity_e(T,I)              'Stationarity condition: x_f_i'

eq_aux1_a(T,I)                      'Auxiliary equation for linearisation of complementarity condition'
eq_aux1_b(T,I)                      'Auxiliary equation for linearisation of complementarity condition'
eq_aux1_c(T,I)                      'Auxiliary equation for linearisation of complementarity condition'
eq_aux1_d(T,I)                      'Auxiliary equation for linearisation of complementarity condition'
eq_aux1_e(T,I)                      'Auxiliary equation for linearisation of complementarity condition'
eq_aux2_a(T,I)                      'Auxiliary equation for linearisation of complementarity condition'
eq_aux2_b(T,I)                      'Auxiliary equation for linearisation of complementarity condition'
eq_aux2_c(T,I)                      'Auxiliary equation for linearisation of complementarity condition'
eq_aux2_d(T,I)                      'Auxiliary equation for linearisation of complementarity condition'
eq_aux2_e(T,I)                      'Auxiliary equation for linearisation of complementarity condition'
eq_aux3_a(T,I)                      'Auxiliary equation for linearisation of complementarity condition'
eq_aux3_b(T,I)                      'Auxiliary equation for linearisation of complementarity condition'
eq_aux3_c(T,I)                      'Auxiliary equation for linearisation of complementarity condition'
eq_aux3_d(T,I)                      'Auxiliary equation for linearisation of complementarity condition'
eq_aux3_e(T,I)                      'Auxiliary equation for linearisation of complementarity condition'

$ifi %scenario% == S4   eq_price_wh(DD)  'Linear relationship for waste-heat price (€/MWh)'
$ifi %scenario% == S5   eq_price_wh(DD)  'Linear relationship for waste-heat price (€/MWh)'
* $ifi %scenario% == S4   eq_price_wh(WW)  'Linear relationship for waste-heat price (€/MWh)'
* $ifi %scenario% == S5   eq_price_wh(WW)  'Linear relationship for waste-heat price (€/MWh)'
* $ifi %scenario% == S4   eq_price_wh(MM)  'Linear relationship for waste-heat price (€/MWh)'
* $ifi %scenario% == S5   eq_price_wh(MM)  'Linear relationship for waste-heat price (€/MWh)'
;
...
eq_obj..                                        obj     =e= + sum((T,J),            C_f_j(T,J)          *x_f_j(T,J))
                                                            ...
                                                            + sum((T),              D_c(T)              *lambda_a(T))
                                                            + sum((T,I)$I_CO(I),    F_a_i(T,I)*Y_c(I)   *mu_a(T,I))
                                                            + sum((T,I)$I_HR(I),    F_a_i(T,I)*Y_h(I)   *mu_b(T,I))   
;
...
$ifi %scenario% == S4       eq_price_wh(DD)..    pi_hr(DD) =e= pi_alpha_0 + pi_alpha_1*sum(T$T_DD_map(T,DD), pi_e(T))/sum(T$T_DD_map(T,DD), 1);
$ifi %scenario% == S5       eq_price_wh(DD)..    pi_hr(DD) =e= pi_alpha_0 + pi_alpha_1*sum(T$T_DD_map(T,DD), Tp_air(T))/sum(T$T_DD_map(T,DD), 1);
* $ifi %scenario% == S4       eq_price_wh(WW)..    pi_hr(WW) =e= pi_alpha_0 + pi_alpha_1*sum(T$T_WW_map(T,WW), pi_e(T))/sum(T$T_WW_map(T,WW), 1);
* $ifi %scenario% == S5       eq_price_wh(WW)..    pi_hr(WW) =e= pi_alpha_0 + pi_alpha_1*sum(T$T_WW_map(T,WW), Tp_air(T))/sum(T$T_WW_map(T,WW), 1);
* $ifi %scenario% == S4       eq_price_wh(MM)..    pi_hr(MM) =e= pi_alpha_0 + pi_alpha_1*sum(T$T_MM_map(T,MM), pi_e(T))/sum(T$T_MM_map(T,MM), 1);
* $ifi %scenario% == S5       eq_price_wh(MM)..    pi_hr(MM) =e= pi_alpha_0 + pi_alpha_1*sum(T$T_MM_map(T,MM), Tp_air(T))/sum(T$T_MM_map(T,MM), 1);
...
eq_stationarity_c(T,I)..                        C_c(I)$I_CO(I) + lambda_a(T) + lambda_b(T,I) + mu_a(T,I)$I_CO(I)                =e= mu_c(T,I);
eq_stationarity_e(T,I)..                        C_f_i(T,I) - eta_i(T,I)*lambda_b(T,I) - (eta_i(T,I)+1)*lambda_c(T,I)$I_HR(I)    =e= mu_e(T,I);

$ifi %scenario% == S2   eq_stationarity_d(T,I)$I_HR(I)..    C_hr(I) -                        pi_hr(T)  + lambda_c(T,I) + mu_b(T,I)  =e= mu_d(T,I);
$ifi %scenario% == S3   eq_stationarity_d(T,I)$I_HR(I)..    C_hr(I) -                        pi_hr     + lambda_c(T,I) + mu_b(T,I)  =e= mu_d(T,I);
$ifi %scenario% == S4   eq_stationarity_d(T,I)$I_HR(I)..    C_hr(I) - sum(DD$T_DD_map(T,DD),pi_hr(DD)) + lambda_c(T,I) + mu_b(T,I)  =e= mu_d(T,I);
$ifi %scenario% == S5   eq_stationarity_d(T,I)$I_HR(I)..    C_hr(I) - sum(DD$T_DD_map(T,DD),pi_hr(DD)) + lambda_c(T,I) + mu_b(T,I)  =e= mu_d(T,I);

* $ifi %scenario% == S4   eq_stationarity_d(T,I)$I_HR(I)..    C_hr(I) - sum(WW$T_WW_map(T,WW),pi_hr(WW)) + lambda_c(T,I) + mu_b(T,I)  =e= mu_d(T,I);
* $ifi %scenario% == S5   eq_stationarity_d(T,I)$I_HR(I)..    C_hr(I) - sum(WW$T_WW_map(T,WW),pi_hr(WW)) + lambda_c(T,I) + mu_b(T,I)  =e= mu_d(T,I);
* $ifi %scenario% == S4   eq_stationarity_d(T,I)$I_HR(I)..    C_hr(I) - sum(MM$T_MM_map(T,MM),pi_hr(MM)) + lambda_c(T,I) + mu_b(T,I)  =e= mu_d(T,I);
* $ifi %scenario% == S5   eq_stationarity_d(T,I)$I_HR(I)..    C_hr(I) - sum(MM$T_MM_map(T,MM),pi_hr(MM)) + lambda_c(T,I) + mu_b(T,I)  =e= mu_d(T,I);


*--- Linearization (SOS) ---*
eq_aux1_a(T,I)$I_CO(I)..                        (mu_a(T,I) + (F_a_i(T,I)*Y_c(I) - x_c(T,I)))/2                                  =e= u_a(T,I); 
eq_aux1_b(T,I)$I_HR(I)..                        (mu_b(T,I) + (F_a_i(T,I)*Y_h(I) - x_hr(T,I)))/2                                 =e= u_b(T,I);
eq_aux1_c(T,I)..                                (mu_c(T,I) + (x_c(T,I)))/2                                                      =e= u_c(T,I);
eq_aux1_d(T,I)$I_HR(I)..                        (mu_d(T,I) + (x_hr(T,I)))/2                                                     =e= u_d(T,I);
eq_aux1_e(T,I)..                                (mu_e(T,I) + (x_f_i(T,I)))/2                                                    =e= u_e(T,I);

eq_aux2_a(T,I)$I_CO(I)..                        (mu_a(T,I) - (F_a_i(T,I)*Y_c(I) - x_c(T,I)))/2                                  =e= nu_a(T,I,'S1') - nu_a(T,I,'S2'); 
eq_aux2_b(T,I)$I_HR(I)..                        (mu_b(T,I) - (F_a_i(T,I)*Y_h(I) - x_hr(T,I)))/2                                 =e= nu_b(T,I,'S1') - nu_b(T,I,'S2');
eq_aux2_c(T,I)..                                (mu_c(T,I) - (x_c(T,I)))/2                                                      =e= nu_c(T,I,'S1') - nu_c(T,I,'S2');
eq_aux2_d(T,I)$I_HR(I)..                        (mu_d(T,I) - (x_hr(T,I)))/2                                                     =e= nu_d(T,I,'S1') - nu_d(T,I,'S2');
eq_aux2_e(T,I)..                                (mu_e(T,I) - (x_f_i(T,I)))/2                                                    =e= nu_e(T,I,'S1') - nu_e(T,I,'S2');

eq_aux3_a(T,I)$I_CO(I)..                        u_a(T,I) - (nu_a(T,I,'S1') + nu_a(T,I,'S2'))                                    =e= 0;
eq_aux3_b(T,I)$I_HR(I)..                        u_b(T,I) - (nu_b(T,I,'S1') + nu_b(T,I,'S2'))                                    =e= 0;
eq_aux3_c(T,I)..                                u_c(T,I) - (nu_c(T,I,'S1') + nu_c(T,I,'S2'))                                    =e= 0;
eq_aux3_d(T,I)$I_HR(I)..                        u_d(T,I) - (nu_d(T,I,'S1') + nu_d(T,I,'S2'))                                    =e= 0;
eq_aux3_e(T,I)..                                u_e(T,I) - (nu_e(T,I,'S1') + nu_e(T,I,'S2'))                                    =e= 0;
...
* Solve statements
* ======================================================================
* --- Uncomment the following three lines to run the grid search loop ---
* $ifi %scenario% == S2 $include 'solve-s2.inc'
* $ifi %scenario% == S3 $include 'solve-s3.inc'
* $ifi %scenario% == S4 $include 'solve-s4.inc'

* --- Uncomment the following line to run the optimal solution, after the optimal price has been found ---
* $include 'solve-optimal.inc'
...
* NOT IMPLEMENTED FOR S5
OUT_OBJ                     = obj.l;
OUT_EHR_PRICE(T)            = 
$ifi %scenario% == S2         pi_hr.l(T);
$ifi %scenario% == S3         pi_hr.l;
$ifi %scenario% == S4         sum(DD, pi_hr.l(DD)$T_DD_map(T,DD));
...
OUT_DH_EHR_PURCHASE_COST    = 
$ifi %scenario% == S2         sum((T,I)$I_HR(I),    pi_hr.l(T)  * x_hr.l(T,I));
$ifi %scenario% == S3         sum((T,I)$I_HR(I),    pi_hr.l     * x_hr.l(T,I));
$ifi %scenario% == S4         sum((T,DD,I)$(T_DD_map(T,DD) and I_HR(I)), pi_hr.l(DD) * x_hr.l(T,I));
...
check_a(T,I)$I_CO(I)    = abs(
                            mu_a.l(T,I) * (F_a_i(T,I)*Y_c(I) - x_c.l(T,I))
);
check_b(T,I)$I_HR(I)    = abs(
                            mu_b.l(T,I) * (F_a_i(T,I)*Y_h(I) - x_hr.l(T,I))
);
check_c(T,I)            = abs(
                            mu_c.l(T,I) * (x_c.l(T,I))
);
check_d(T,I)$I_HR(I)    = abs(
                            mu_d.l(T,I) * (x_hr.l(T,I))
);
check_e(T,I)            = abs(
                            mu_e.l(T,I) * (x_f_i.l(T,I))
);

*--- calculate maximum and sum ---*
max_a   = smax((T,I)$I_CO(I),    check_a(T,I));
max_b   = smax((T,I)$I_HR(I),    check_b(T,I));
max_c   = smax((T,I),            check_c(T,I));
max_d   = smax((T,I)$I_HR(I),    check_d(T,I));
max_e   = smax((T,I),            check_e(T,I));      
sum_a   = sum((T,I)$I_CO(I),     check_a(T,I));
sum_b   = sum((T,I)$I_HR(I),     check_b(T,I));
sum_c   = sum((T,I),             check_c(T,I));
sum_d   = sum((T,I)$I_HR(I),     check_d(T,I));
sum_e   = sum((T,I),             check_e(T,I));
...
display obj.l;
display pi_hr.l;
$ifi %scenario% == S4   display pi_alpha_0.l, pi_alpha_1.l;
$ifi %scenario% == S5   display pi_alpha_0.l, pi_alpha_1.l;

display "Complementarity checks"
display max_a, max_b, max_c, max_d, max_e, sum_a, sum_b, sum_c, sum_d, sum_e;
