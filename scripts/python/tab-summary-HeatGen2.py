...
output_path = f'M:/J2/results/selected_weeks/consolidated/Summary_HeatGenFuel_{CASE}.xlsx'
data_all    = pd.read_csv(file_path)

# Rename and sort the TYPE column
data_all['TYPE'] = data_all['TYPE'].replace(rename_type)
data_all['F']    = data_all['F'].replace(rename_fuel)
...
# Pivot the table
data_all = data_all.pivot_table(index=['scenario'], columns=['F'], values='value', aggfunc='sum', fill_value=0)*MULTIPLIER
